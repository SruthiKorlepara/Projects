...
class ticket:
	def __init__(self,train,user,ticket_num,coach):
		self.pnr = str(train.num)+str(user.userid)+str(random.randint(100,999))
...
class user:
	def __init__(self,userid = 0,name = '',hometown = '',cell_num = '',password = ''):
		self.userid = userid
		self.name = name
		self.hometown = ''
		self.cell_num = ''
...
class acceptors:
	...
	def accept_userid():
		...
		try:
			...
			return acceptors.accept_userid()
		else:
			return userid
...
def book_ticket():
	...
	if trains[choice].check_availabilty(coach,ticket_num):
		...
		if prompt == 'y':
			...
			tick = ticket(trains[choice],users[userid],ticket_num,coach)
	...
	if pnr in ticket_dict:
		...
		if prompt == 'y':
			if logged_in:
				...
				del users[ticket_dict[pnr].userid].history[pnr]
				del ticket_dict[pnr]
			else:
				login('p')
				print("Ticket Cancelled.\n")
...
def create_new_acc():
	...
	u = user(userid, user_name, hometown, cell_num, password)
	print("Your user ID is :- ",userid)
	users.update({u.userid : u})
	menu()
...
def login(flag = ''):
	global userid
	global password
	userid = acceptors.accept_userid()
...
t1 = train('Gouthami Express',12737,'12:34','22:12','vsk','hyd','Wed',30,23,43,2205,320,234)
t2 = train('Howrah Express',12565,'02:34','23:12','che','kol','Mon',33,4,12,3434,435,234)
t3 = train('Vande Bharat',22353,'11:56','03:12','hyd','vsk','Fri',33,24,77,455,325,533)
trains = {t1.num:t1,t2.num:t2,t3.num:t3}
u1 = user(1765,'ram','rajahmundry','903802','ram')
u2 = user(2568,'bheem','hyderabad','945210','bheem')
users={u1.userid : u1, u2.userid : u2}
ticket_dict = {}
